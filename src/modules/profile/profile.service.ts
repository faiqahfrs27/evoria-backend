...
export class ProfileService {
  constructor(private prisma: PrismaClient, private cloudinaryService: CloudinaryService) {}
  ...
  updateProfilePic = async (userId: string, profilePic: Express.Multer.File) => {
    const user = await this.prisma.user.findFirst({
      where: { id: userId, deletedAt: null },
    });
    ...
    const { secure_url } = await this.cloudinaryService.upload(profilePic) as { secure_url: string };
